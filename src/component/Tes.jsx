...
const Tes = () => {
    ...
    const changecontent = (DataMobil) => {
        setpopupcontent([DataMobil]);
    };
    ...
    return(
        <div className="text-black">
            {DataMobil.map((item)=>(
                    ...
                    <div className="flex">
                        <a className="absolute inset-x-0 bottom-1 lg:bottom-4"><div className="flex justify-center">
                            <button onClick={()=> changecontent(DataMobil)} className="bg-abu text-sm text-white font-bold py-3 px-4 rounded-full">Klik Untuk Lebih Detail</button></div></a>
                    </div>
                </div>
                ))}
            {/* <Modal open={popupcontent} /> */}
            <div>
                <button>X</button>
            </div>
            <ul>
